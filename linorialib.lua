...
local Library = {
    ...
    FontColor = Color3.fromRGB(227, 233, 239);
    MainColor = Color3.fromRGB(21, 21, 21);
    BackgroundColor = Color3.fromRGB(12, 12, 12);
    AccentColor = Color3.fromRGB(81, 213, 134);
...
function Library:CreateWindow(...)
    ...
    local function extendUDim2(base, xOffset, yOffset)
        return UDim2.new(base.X.Scale, base.X.Offset + xOffset, base.Y.Scale, base.Y.Offset + yOffset);
    end

    local WindowShadow = Library:Create('ImageLabel', {
        Name = 'gamesenseShadow';
        AnchorPoint = Config.AnchorPoint;
        BackgroundTransparency = 1;
        ...
        Size = extendUDim2(Config.Size, 70, 70);
        Position = extendUDim2(Config.Position, 0, 12);
        Visible = false;
        ZIndex = 0;
        Parent = ScreenGui;
    });
    ...
    local function syncShadow()
        WindowShadow.Position = extendUDim2(Outer.Position, 0, 12);
        WindowShadow.Size = extendUDim2(Outer.Size, 70, 70);
    ...
    Library:GiveSignal(Outer:GetPropertyChangedSignal('AnchorPoint'):Connect(syncShadow));
    Library:GiveSignal(Outer:GetPropertyChangedSignal('Parent'):Connect(function()
        if not Outer.Parent then
            WindowShadow:Destroy();
        end
    end));
    ...
    local function buildTitleGradientSequence()
        local innerShade = Library:GetDarkerColor(Library.MainColor);
        local edgeShade = Library:GetDarkerColor(innerShade);

        return ColorSequence.new({
            ColorSequenceKeypoint.new(0, edgeShade),
            ColorSequenceKeypoint.new(0.5, Library.MainColor),
            ColorSequenceKeypoint.new(1, edgeShade)
        });
    end
    ...
    Library:AddToRegistry(TitleGradient, {
        Color = function()
            return buildTitleGradientSequence();
        end;
    });
